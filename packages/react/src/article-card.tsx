...
// The payload strip measures its own overflow and scrolls itself, so the module
// is a client boundary. ArticleCard was server-renderable until 2026-08-28; the
// card still server-renders as a client component, but a consumer holding a
// blog index now ships it to the browser.
import { forwardRef, useEffect, useId, useRef, useState, type HTMLAttributes, type ReactNode } from "react";
import { cx } from "./utils";
import { ChevronLeftIcon, ChevronRightIcon, DatasetIcon, DotIcon, DownloadIcon, InteractiveIcon, LAMP_WEIGHT } from "./icons";
...
function assetGlyph(kind: ArticleCardAssetKind): ReactNode {
 ...
 */
function assetRows(assets: ArticleCardAsset[]): ArticleCardAsset[][] {
  const perRow = Math.ceil(assets.length / ASSET_ROWS);
  const rows: ArticleCardAsset[][] = [];
  for (let index = 0; index < assets.length; index += perRow) rows.push(assets.slice(index, index + perRow));
  return rows;
}

function ArticleCardPayload({ assets, label }: { assets: ArticleCardAsset[]; label: ReactNode }) {
  ...
  return (
    <div className="td-mk-article-payload">
      ...
      <div className="td-mk-article-assets" ref={scroller} role="group" aria-labelledby={labelId}>
        {assetRows(assets).map((row, rowIndex) => (
          <ul className="td-mk-article-assetrow" key={rowIndex}>
            {row.map((asset, index) => (
              <li className="td-mk-article-asset-item" key={index}>
                ...
                >
                  <span className="td-mk-article-asset-glyph" aria-hidden="true">{asset.icon ?? assetGlyph(asset.kind)}</span>
...
export interface ArticleCardGridProps extends HTMLAttributes<HTMLDivElement> {}

export const ArticleCardGrid = forwardRef<HTMLDivElement, ArticleCardGridProps>(function ArticleCardGrid(
  { className, ...props }, ref,
) {
