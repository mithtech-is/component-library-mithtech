 ...
 */
export const ReadingProgress = forwardRef<HTMLDivElement, ReadingProgressProps>(function ReadingProgress(
  ...
  useEffect(() => {
    const measure = () => {
      ...
      if (target && !element) {
        ...
        console.warn(`ReadingProgress: no element matches \`${target}\`; falling back to the document.`);
      }
  ...
  return (
    <div
      {...props}
      ref={ref}
      className={cx("td-react-readingprogress", `td-react-readingprogress--${tone}`, className)}
      role="progressbar"
      aria-label={label}
      aria-valuemin={0}
      aria-valuemax={100}
    ...
    >
      <div className="td-react-readingprogress-bar" />
    </div>
  );
});
