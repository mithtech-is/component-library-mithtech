...
// thing anyone wants.
export interface PageNavProps extends Omit<HTMLAttributes<HTMLElement>, "title"> {
  items: PageNavItem[];
...
export const PageNav = forwardRef<HTMLElement, PageNavProps>(function PageNav(
  ...
  return (
    <div
      {...props}
      ref={ref as never}
      className={cx("td-react-pagenav", sticky && "td-react-pagenav--sticky", className)}
    >
      {hasIdentity ? (
        <div className="td-react-pagenav-identity">
          {eyebrow ? <p className="td-react-pagenav-eyebrow">{eyebrow}</p> : null}
          {title ? <p className="td-react-pagenav-title">{title}</p> : null}
        </div>
      ) : null}

      <nav className="td-react-pagenav-scroll" aria-label={label}>
        <div className="td-react-pagenav-tabs">
          {items.map((item, index) => {
            const tabClass = cx("td-react-pagenav-tab");
            ...
            const body = (
              <>
                <span className="td-react-pagenav-tab-label">{item.label}</span>
            ...
            if (renderLink) {
              return (
                <span className="td-react-pagenav-slot" key={`${item.href}-${index}`}>
      ...
      {actions ? <div className="td-react-pagenav-actions">{actions}</div> : null}
    </div>
  );
});
