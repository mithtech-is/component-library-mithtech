...
import "./tonaldepth-confirm-button.css";
import { TonalDepthButton } from "./tonaldepth-button";

export interface TonalDepthConfirmButtonProps extends Omit<ButtonProps, "children" | "onClick"> {
 ...
 *
 * This is `TonalDepthButton` with a state machine on top, in its own module for the
 * reason every wrapper here exists: `TonalDepthButton` is server-renderable and is on
 ...
 */
export const TonalDepthConfirmButton = forwardRef<HTMLButtonElement, TonalDepthConfirmButtonProps>(function TonalDepthConfirmButton(
  ...
  const press = (event: MouseEvent<HTMLButtonElement>) => {
    if (!armed) {
      // `preventDefault` because a TonalDepthConfirmButton inside a form must not submit
  ...
  return (
    <TonalDepthButton
      ...
      className={armed ? "td-registry-confirm td-registry-confirm--armed" : "td-registry-confirm"}
