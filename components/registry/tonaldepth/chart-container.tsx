...
import "./tonaldepth-chart-container.css";
...
export interface TonalDepthChartLegendItem { label: ReactNode; color?: string }

export interface TonalDepthChartContainerProps extends Omit<HTMLAttributes<HTMLElement>, "title"> {
  title: ReactNode;
  meta?: ReactNode;
  description?: string;
  legend?: TonalDepthChartLegendItem[];
  children: ReactNode;
}
 ...
 * A chart, housed.
 *
 * This is a `TonalDepthFrame` with the slots a plot needs already wired: the caption in
 ...
 * in a well rather than offered on a plate — that pairing is `TonalDepthFrame`'s, and
 ...
 */
export const TonalDepthChartContainer = forwardRef<HTMLElement, TonalDepthChartContainerProps>(function TonalDepthChartContainer(
  ...
  return (
    <TonalDepthFrame
      ...
      className={cx("td-registry-chart", className)}
    >
      {description ? <p id={descriptionId} className="td-registry-visually-hidden">{description}</p> : null}
