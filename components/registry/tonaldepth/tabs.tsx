...
export const TonalDepthTabs = forwardRef<HTMLDivElement, TonalDepthTabsProps>(function TonalDepthTabs(
  { items, value, defaultValue, onValueChange, label = "TonalDepthTabs", orientation = "horizontal", className, ...props }, ref,
) {
  const id = useId();
  ...
  return (
    <div {...props} ref={ref} data-orientation={orientation} className={cx("td-tabs", "td-registry-tabs", className)}>
