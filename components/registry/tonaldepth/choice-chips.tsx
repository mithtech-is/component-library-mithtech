...
import "./tonaldepth-choice-chips.css";

function cx(...values: Array<string | false | null | undefined>): string {
  return values.filter(Boolean).join(" ");
}

export interface TonalDepthChoiceOption {
  value: string;
  label: ReactNode;
  /**
...
export interface TonalDepthChoiceChipsProps extends Omit<HTMLAttributes<HTMLDivElement>, "onChange" | "defaultValue"> {
  options: TonalDepthChoiceOption[];
  /**
 ...
 * `TonalDepthChoiceChips` is a plain input, single or multi, with no result set implied.
 * **Not `Chip`/`LinkChip`**, which are a static token and a navigable pill; these
 * chips toggle a selection.
 */
export const TonalDepthChoiceChips = forwardRef<HTMLDivElement, TonalDepthChoiceChipsProps>(function TonalDepthChoiceChips(
  props,
  ref,
) {
  const {
  ...
  return (
    <div {...domProps} ref={ref} role="group" aria-label={label} className={cx("td-registry-choicechips", className)}>
      {options.map((option) => {
