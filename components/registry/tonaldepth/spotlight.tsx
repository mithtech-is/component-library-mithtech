...
import "./tonaldepth-spotlight.css";

function cx(...values: Array<string | false | null | undefined>): string {
  return values.filter(Boolean).join(" ");
}

export interface TonalDepthSpotlightAction {
  id: string;
  label: ReactNode;
...
export interface TonalDepthSpotlightProps extends Omit<HTMLAttributes<HTMLDivElement>, "onSelect" | "children"> {
  open: boolean;
   ...
   */
  actions: TonalDepthSpotlightAction[];
  query?: string;
  onQueryChange?: (query: string) => void;
  onSelect?: (action: TonalDepthSpotlightAction) => void;
  placeholder?: string;
 ...
 */
export const TonalDepthSpotlight = forwardRef<HTMLDivElement, TonalDepthSpotlightProps>(function TonalDepthSpotlight(
  ...
  const groups = useMemo(() => {
    const out: { name?: string; rows: TonalDepthSpotlightAction[] }[] = [];
  ...
  const choose = (action: TonalDepthSpotlightAction) => {
    onSelect?.(action);
    onOpenChange(false);
  };
  ...
  return (
    <div
      className="td-registry-spotlight-scrim"
      data-origin={origin}
    ...
    >
      <div
        {...props}
        ref={ref}
        className={cx("td-registry-spotlight", className)}
        role="dialog"
        aria-modal="true"
        aria-label={label}
        onKeyDown={keys}
      >
        <div className="td-registry-spotlight-field">
          <input
            ref={input}
            className="td-registry-spotlight-input"
            type="text"
            value={text}
          ...
          />
          <kbd className="td-registry-spotlight-esc">esc</kbd>
        </div>
        <div className="td-registry-spotlight-results" id={listId} role="listbox" aria-label={label}>
          {actions.length === 0 ? <p className="td-registry-spotlight-empty">{empty}</p> : groups.map((group, index) => (
            <div className="td-registry-spotlight-group" key={index}>
              {group.name !== undefined ? <p className="td-registry-spotlight-grouplabel">{group.name}</p> : null}
              {group.rows.map(action => {
                ...
                return (
                  <div
                    className="td-registry-spotlight-row"
                    id={`${listId}-${action.id}`}
                    role="option"
                  ...
                  >
                    {action.icon !== undefined ? <span className="td-registry-spotlight-icon" aria-hidden="true">{action.icon}</span> : null}
                    <span className="td-registry-spotlight-body">
                      <span className="td-registry-spotlight-label">{action.label}</span>
                      {action.detail !== undefined ? <span className="td-registry-spotlight-detail">{action.detail}</span> : null}
                    </span>
                    {action.shortcut ? <kbd className="td-registry-spotlight-kbd">{action.shortcut}</kbd> : null}
                  </div>
                );
              })}
            </div>
        ...
        <div className="td-registry-spotlight-foot">
