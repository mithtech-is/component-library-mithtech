 ...
 * channel TonalDepthtones carry each platform's own fill.
 *
 ...
 */
export type TonalDepthIconButtonTone =
...
const TonalDepthtones: Record<TonalDepthIconButtonTone, string> = {
  neutral: "",
...
interface TonalDepthIconButtonOwnProps {
...
export type TonalDepthIconButtonProps = TonalDepthIconButtonOwnProps &
...
export const TonalDepthIconButton = forwardRef<HTMLButtonElement & HTMLAnchorElement, TonalDepthIconButtonProps>(
  function TonalDepthIconButton({ icon, label, tone = "neutral", href, renderLink, glow, className, type, ...props }, ref) {
    ...
    if (!hasLabel && !props["aria-label"] && !props["aria-labelledby"]) {
      ...
      console.warn("TonalDepthIconButton: an icon-only button needs `aria-label` (or `aria-labelledby`), or a `label`.");
    }
    const classes = cx("td-glowicon", "td-registry-iconbutton", TonalDepthtones[tone], hasLabel && "td-glowicon--text", className);
    const style = glow ? { ...props.style, ["--td-lamp-glow" as string]: glow } : props.style;
    const content = (
      <>
        <span className="td-registry-iconbutton-lamp" aria-hidden="true">{icon}</span>
