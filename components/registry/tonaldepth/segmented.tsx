...
import "./tonaldepth-segmented.css";

function cx(...values: Array<string | false | null | undefined>): string {
  return values.filter(Boolean).join(" ");
}

export interface TonalDepthSegmentedOption {
  value: string;
  label: ReactNode;
  disabled?: boolean;
}

export interface TonalDepthSegmentedProps extends Omit<HTMLAttributes<HTMLDivElement>, "onChange"> {
  /** The choices, drawn left to right. */
  options: TonalDepthSegmentedOption[];
 ...
 * multi-select.
 */
export const TonalDepthSegmented = forwardRef<HTMLDivElement, TonalDepthSegmentedProps>(function TonalDepthSegmented(
  ...
  return (
    <div
      ...
      role="group"
      aria-label={label}
      className={cx("td-segmented", "td-registry-segmented", className)}
    >
      {options.map((option) => (
        <button
          key={option.value}
