...
import "./tonaldepth-linkchip.css";

function cx(...values: Array<string | false | null | undefined>): string {
  return values.filter(Boolean).join(" ");
}

export interface TonalDepthLinkChipProps extends Omit<AnchorHTMLAttributes<HTMLAnchorElement>, "href"> {
  /** Where the pill goes. A TonalDepthLinkChip without a destination is a `Chip`. */
  href: string;
  /**
 ...
 * when only its × does anything ([[L12]]). `TonalDepthLinkChip` is the opposite case — a
 ...
 * prose; `TonalDepthLinkChip` is a pill in a row of pills — a tag rail under an article, an
 ...
 */
export const TonalDepthLinkChip = forwardRef<HTMLAnchorElement, TonalDepthLinkChipProps>(function TonalDepthLinkChip(
  { href, renderLink, leading, className, children, style, ...props },
  ref,
) {
  const classes = cx("td-chip", "td-registry-linkchip", className);
  const content = (
    <>
      {leading != null ? (
        <span className="td-registry-linkchip-lead" aria-hidden>
          {leading}
        </span>
      ) : null}
      {children}
