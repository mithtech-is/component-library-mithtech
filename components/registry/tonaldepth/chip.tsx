...
import "./tonaldepth-chip.css";

function cx(...values: Array<string | false | null | undefined>): string {
  return values.filter(Boolean).join(" ");
}

export interface TonalDepthChipProps extends HTMLAttributes<HTMLSpanElement> {
  /**
 ...
 * options with counts that a reader chooses *from*. `TonalDepthChip` is a single token you
 ...
 */
export const TonalDepthChip = forwardRef<HTMLSpanElement, TonalDepthChipProps>(function TonalDepthChip(
  { onRemove, removeLabel, className, children, ...props },
  ref,
) {
  return (
    <span {...props} ref={ref} className={cx("td-chip", "td-registry-chip", className)}>
      {children}
      {onRemove ? (
        <button
          type="button"
