...
import "./rect-title.css";

export type RectTitleLevel = 1 | 2 | 3;
 ...
 */
export type RectTitleTone = "ink" | "muted" | "brand" | "accent" | "green" | "error";

/** How far a line may be scaled up to reach the longest one. */
const DEFAULT_MAX_SCALE = 1.5;

export interface RectTitleProps extends Omit<HTMLAttributes<HTMLHeadingElement>, "children"> {
  /**
...
const toneClass: Record<RectTitleTone, string> = {
  ink: "",
...
function lineTone(tones: RectTitleTone[] | undefined, accent: boolean, i: number, count: number): RectTitleTone {
 ...
 */
export function splitRectTitle(text: string, maxLines = 3, exactLines?: number): string[] {
  ...
  return balanceLines(words, lineCount);
}
...
function inkSpan(text: string): { start: number; end: number } {
  const leading = /^[^\p{L}\p{N}]*/u.exec(text)?.[0].length ?? 0;
  const trailing = /[^\p{L}\p{N}]*$/u.exec(text)?.[0].length ?? 0;
  const start = leading;
  const end = Math.max(start, text.length - trailing);
  // A line that is nothing but punctuation keeps its whole width — there is no
  // ink to align, so trimming it to zero would divide by nothing below.
  return end > start ? { start, end } : { start: 0, end: text.length };
}
...
function balanceLines(words: string[], lineCount: number): string[] {
  ...
  const inkLength = (text: string) => text.replace(EDGE_PUNCTUATION, "").length;
 ...
 */
function fitLines(heading: HTMLElement, maxScale: number): boolean {
 ...
 * of the component.
 */
export const RectTitle = forwardRef<HTMLHeadingElement, RectTitleProps>(function RectTitle(
  ...
  const written = Array.isArray(lineSpec)
    ...
    : null;
  const lines = written ?? splitRectTitle(text ?? "", maxLines, typeof lineSpec === "number" ? lineSpec : undefined);
  if (!lines.length) {
    ...
    console.warn("RectTitle: pass `text`, or a `lines` array. Neither is set, so the heading is empty.");
  }
  const scaleCeiling = maxScale ?? (forceRectangle ? Infinity : DEFAULT_MAX_SCALE);
  ...
  useFitEffect(() => {
    ...
    const run = () => {
      ...
      if (!fitLines(heading.current, scaleCeiling) && attempts++ < 20) frame = requestAnimationFrame(run);
    };
    const refit = () => {
      attempts = 0;
      run();
  ...
  return (
    <Heading {...props} ref={setRefs} className={cx(`td-${Heading}`, "td-react-rect-title", className)}>
      {lines.map((line, i) => (
        <Fragment key={`${line}-${i}`}>
          ...
          {i > 0 ? " " : null}
          <span className={cx("td-h1-line", toneClass[lineTone(tones, accent, i, lines.length)])}>
            {line}
          </span>
        </Fragment>
      ))}
      ...
      {srSuffix ? <span className="td-react-rect-title-sr">{srSuffix}</span> : null}
    </Heading>
  );
});
