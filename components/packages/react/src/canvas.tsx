...
import { FitIcon, LAMP_WEIGHT, MaximizeIcon, MinimizeIcon, ResetIcon, SoundOffIcon, SoundOnIcon, ZoomInIcon, ZoomOutIcon } from "./icons";
import "./canvas.css";
...
export interface CanvasFlyOptions {
...
export interface CanvasApi {
  ...
  flyTo: (box: CanvasBox, options?: CanvasFlyOptions) => void;
  /** Frame the whole `world` box — what the fit control does. */
  fit: (options?: CanvasFlyOptions) => void;
 ...
 */
export type CanvasWheelMode = "modifier" | "free" | "off";

export interface CanvasSoundControl {
  muted: boolean;
  onToggle: () => void;
}

export interface CanvasControlsProps extends Omit<HTMLAttributes<HTMLDivElement>, "onSelect"> {
  /** The camera these drive. The same ref the `Canvas` was given. */
  camera: { current: CanvasApi | null };
 ...
 *
 * Exported separately from `Canvas` because a host that draws its own chrome
 ...
 *
 * Every button is an `IconButton` carrying a filled glyph from the library's
 * own set, so each one is a lamp ([[L15]]) and none of them is a bare
 * `<button>` with somebody else's mark inside it.
 */
export const CanvasControls = forwardRef<HTMLDivElement, CanvasControlsProps>(function CanvasControls(
  { camera, step = 1.3, fit = true, onReset, sound, fullscreen, direction = "column", labels, className, children, ...props },
  ref,
) {
  const name = { ...CONTROL_LABELS, ...labels };
  return (
    <div
      {...props}
      ref={ref}
      className={cx("td-react-canvas-controls", `td-react-canvas-controls--${direction}`, className)}
    >
      <IconButton
...
/* `title` is a ReactNode here, so the DOM attribute of the same name — which
   is a tooltip string — is dropped rather than fought with. Frame does the
   same, for the same reason. */
export interface CanvasProps extends Omit<HTMLAttributes<HTMLElement>, "onSelect" | "title"> {
   ...
   *  `CanvasControls` in its own chrome. */
  controls?: boolean;
  sound?: CanvasSoundControl;
  /** Handed to the built-in cluster — see `CanvasControlsProps`. */
  onReset?: () => void;
   ...
   * the canvas draws its own `Frame` around itself and moves the control
   * cluster into that frame's head — which is where a housed map's controls
  ...
  footnote?: ReactNode;
  /** Extra controls in the frame's head, beside the cluster. */
  actions?: ReactNode;
  /**
   ...
   * inside a `Frame`, whose carved well is the housing already, and for a
...
function frame(box: CanvasBox, viewport: { w: number; h: number }, options: CanvasFlyOptions, min: number, max: number): CanvasTransform {
 ...
 * `CanvasWheelMode`. A canvas that has taken the display should pass
 * `wheel="free"`.
 *
 ...
 */
export const Canvas = forwardRef<CanvasApi, CanvasProps>(function Canvas(
  {
  ...
  const transform = useRef<CanvasTransform>({ x: 0, y: 0, scale: initialScale });
  ...
  const flyTo = useCallback((box: CanvasBox, options: CanvasFlyOptions = {}) => {
    const vp = viewportRef.current;
    if (!vp) return;
    stop();
    pristine.current = false;
    const size = { w: vp.clientWidth, h: vp.clientHeight };
    const target = frame(box, size, { padding, padRight: panelInset, ...options }, min, max);
  ...
  /* Built here rather than inside `useImperativeHandle`, and this is not a
     ...
     it. */
  const camera = useMemo<CanvasApi>(() => {
    const api: CanvasApi = {
      flyTo,
  ...
  /* A canvas given anything to say about itself draws its own housing. The
     frame's plate carries the identity and the well carries the map, which is
  ...
  /* Where the controls go. Housed, they belong to the frame's head — but in
  ...
  const canvas = (
    <section
      ...
      className={cx(
        ...
        housed && isFullscreen && "td-react-canvas--standalone",
        isFullscreen && "td-react-canvas--fullscreen",
        className,
      )}
      onKeyDown={onKeyDown}
    >
      <div
        ref={attachViewport}
        className={cx("td-react-canvas-viewport", interactive && "td-react-canvas-viewport--interactive")}
      ...
      >
        <div ref={worldRef} className="td-react-canvas-world">{children}</div>
      </div>
      ...
      {cta ? <div className="td-react-canvas-cta">{cta}</div> : null}
      {hint ? (
        ...
        <Badge
          ...
          className="td-react-canvas-hint"
        >
          {hint}
        </Badge>
      ) : null}
  ...
  /* In fullscreen the canvas is the whole screen, so the frame around it has
  ...
  return (
    <Frame
      ...
      footnote={footnote}
      actions={actions || cluster ? <div className="td-react-canvas-head">{cluster}{actions}</div> : undefined}
      flushContent
    >
      {canvas}
    </Frame>
