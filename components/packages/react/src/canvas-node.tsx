...
export interface CanvasNodeProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, "title"> {
 ...
 * exists.
 */
export const CanvasNode = forwardRef<HTMLButtonElement, CanvasNodeProps>(function CanvasNode(
  ...
  return (
    <button
      ...
      className={cx("td-card-surface", "td-react-canvas-node", selected && "td-react-canvas-node--selected", className)}
    >
      <span className="td-react-canvas-node-title td-display-sm">{title}</span>
      {sub ? <span className="td-react-canvas-node-sub">{sub}</span> : null}
      {children}
    </button>
  );
});
