...
export const Avatar = forwardRef<HTMLSpanElement, AvatarProps>(function Avatar(
  ...
  const mark = initials ?? initialsOf(name);
  return (
    <span
      {...props}
      ref={ref}
      data-size={size}
      className={cx("td-react-avatar", className)}
      title={props.title ?? name}
    >
      {showImage ? (
        <img
          {...imgProps}
          className="td-react-avatar-img"
          src={src}
          alt={name}
        ...
        <span className="td-react-avatar-icon" aria-hidden="true">{icon}</span>
      ) : (
        <span className="td-react-avatar-initials" aria-hidden="true">{mark}</span>
      )}
      {/* The name reaches a screen reader even when the visible content is
          initials or a glyph, both of which are hidden. */}
      {showImage ? null : <span className="td-react-avatar-sr">{name}</span>}
      {presence ? (
        <span className="td-react-avatar-presence" data-presence={presence}>
          <span className="td-react-avatar-presence-lamp" aria-hidden="true" />
          <span className="td-react-avatar-sr">{PRESENCE_LABEL[presence]}</span>
        </span>
      ) : null}
    </span>
  );
...
export const AvatarGroup = forwardRef<HTMLDivElement, AvatarGroupProps>(function AvatarGroup(
  ...
  return (
    <div {...props} ref={ref} data-size={size} className={cx("td-react-avatargroup", className)}>
      {shown.map((person, index) => (
        ...
        <span className="td-react-avatargroup-slot" key={index} style={{ zIndex: shown.length - index }}>
          {person}
        </span>
      ))}
      {hidden > 0 ? (
        <span className="td-react-avatargroup-slot" style={{ zIndex: 0 }}>
          <span className="td-react-avatar td-react-avatar--more" data-size={size}>
            <span className="td-react-avatar-initials">+{hidden}</span>
          </span>
        </span>
      ) : null}
    </div>
