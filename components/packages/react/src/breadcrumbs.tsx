 ...
 */
export type BreadcrumbSeparator = "chevron" | "slash" | "condensed";
...
export const Breadcrumbs = forwardRef<HTMLElement, BreadcrumbsProps>(function Breadcrumbs(
  ...
  return (
    <nav {...props} ref={ref} aria-label={label} className={cx("td-bc", "td-react-bc", `td-react-bc--${separator}`, className)}>
      <ol className="td-react-bc-list">
        {items.map((item, index) => {
          const isCurrent = index === last;
          const itemClass = cx("td-bc-item", "td-react-bc-item");
          return (
            <li className="td-react-bc-crumb" key={index}>
              {index > 0 ? <span className="td-bc-sep td-react-bc-sep" aria-hidden="true" /> : null}
              {isCurrent || !item.href
