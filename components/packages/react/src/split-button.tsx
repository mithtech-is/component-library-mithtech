...
export interface SplitButtonProps extends Omit<ButtonHTMLAttributes<HTMLButtonElement>, "onSelect" | "children" | "type"> {
...
export const SplitButton = forwardRef<HTMLButtonElement, SplitButtonProps>(function SplitButton(
  ...
  const sharedPrimary = {
    className: "td-react-split-primary",
    onClick: () => { if (chosen) onAction?.(chosen); },
  };

  return (
    <div ref={root} className={cx("td-react-split", mustChoose && "td-react-split--unset", className)}>
      {chosen?.href && !primaryDisabled ? (
        renderLink
          ? renderLink({
            ...
            children: (
              <>
                {actions.some(a => a.icon) ? <span className="td-react-split-icon" aria-hidden="true">{chosen.icon}</span> : null}
                <span className="td-react-split-label">{primaryLabel}</span>
              </>
            ),
          })
          : (
      ...
      <button
        ...
        className="td-react-split-toggle"
        onClick={() => setOpen(current => !current)}
      >
        <span className="td-react-split-chevron" aria-hidden="true" />
      </button>
      {open ? (
        <div id={`${base}-menu`} role="menu" aria-label={menuLabel} className="td-react-split-menu" onKeyDown={onMenuKeyDown}>
          {actions.map(action => (
            <button
              key={action.id}
              type="button"
              ...
              className={cx("td-react-split-item", action.id === chosenId && "td-react-split-item--on")}
              onClick={() => choose(action)}
            >
              {action.icon ? <span className="td-react-split-icon" aria-hidden="true">{action.icon}</span> : null}
              <span className="td-react-split-item-text">
                <span className="td-react-split-item-label">{action.label}</span>
                {action.detail ? <span className="td-react-split-item-detail">{action.detail}</span> : null}
              </span>
            </button>
          ))}
        </div>
