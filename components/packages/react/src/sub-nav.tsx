...
export const SubNav = forwardRef<HTMLElement, SubNavProps>(function SubNav(
  ...
  return (
    <nav
      ...
      ref={ref}
      aria-label={label}
      className={cx("td-react-subnav", plate && "td-react-subnav--plate", className)}
    >
      {sections.map((section, index) => (
        <div className="td-sb-section td-react-subnav-section" key={index}>
          {section.label !== undefined ? <div className="td-sb-label">{section.label}</div> : null}
          {section.items.map((item, index) => {
            const body = (
              <>
                {item.icon !== undefined ? <span className="td-react-subnav-icon" aria-hidden="true">{item.icon}</span> : null}
                <span className="td-react-subnav-label">{item.label}</span>
            ...
            const linkProps = {
              className: "td-sb-item td-react-subnav-item",
              href: item.href,
              children: body,
              ...(item.current ? { "aria-current": "page" as const } : {}),
            };
            return renderLink
              ? <span key={`${item.href}-${index}`} className="td-react-subnav-slot">{renderLink(linkProps)}</span>
