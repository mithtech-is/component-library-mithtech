...
export interface CounterProps extends Omit<HTMLAttributes<HTMLDivElement>, "children"> {
 ...
 */
export const Counter = forwardRef<HTMLDivElement, CounterProps>(function Counter(
  ...
  return (
    <div
      ...
      className={cx("td-react-counter", className)}
    >
      <span className="td-react-counter-value">
        ...
        {suffix !== undefined ? <span className="td-react-counter-suffix">{suffix}</span> : null}
      </span>
      <span className="td-react-counter-label">{label}</span>
    </div>
  );
});

export interface CounterRowProps extends HTMLAttributes<HTMLDivElement> {}

/** A row of counters that wraps. Even gaps, and no plate — the numbers are the row. */
export const CounterRow = forwardRef<HTMLDivElement, CounterRowProps>(function CounterRow(
  { className, ...props },
  ref,
) {
  return <div {...props} ref={ref} className={cx("td-react-counter-row", className)} />;
});
