 ...
 *
 * The cells are separate inputs because that is what makes the code legible at
 * a glance and what lets a reader correct the third digit without retyping the
 * other five. The cost is that every gesture a single input gets free has to be
 * rebuilt: paste has to spread across the cells, Backspace on an empty cell has
 * to step back, and the arrows have to walk. All three are here, because a code
 * field that loses a paste is a code field nobody can use.
 */
...
export const OtpInput = forwardRef<HTMLInputElement, OtpInputProps>(function OtpInput(
  ...
  return (
    <div
      className={cx("td-otp", "td-react-otp", className)}
      role="group"
      aria-label={label}
